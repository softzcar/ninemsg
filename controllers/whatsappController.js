...
client.on('qr', (qr) => {
    ...
    qrcode.toDataURL(qr, (err, qrCodeImage) => {
        if (err) {
            ...
            console.log('QR generado correctamente. Escanéalo para vincular el dispositivo.');
        }
    });
});
...
exports.showQRCode = (req, res) => {
    if (qrCodeImage) {
        res.send(`<img src="${qrCodeImage}" alt="Código QR para WhatsApp">`);
    } else {
        res.status(500).json({ message: 'El código QR no está disponible aún, intenta más tarde.' });
    }
};
...
exports.sendMessage = async (req, res) => {
    ...
    if (!global.whatsappReady) {
        return res.status(500).json({ message: 'El cliente de WhatsApp aún no está listo para enviar mensajes.' });
    }

    // Verificamos si los parámetros necesarios están presentes
    if (!phone || !message) {
        return res.status(400).json({ message: 'Faltan parámetros: número de teléfono y/o mensaje.' });
    }
    ...
    try {
        ...
        res.status(200).json({ message: `Mensaje enviado exitosamente a ${name} (${phone})` });
    } catch (error) {
        console.error('Error al enviar el mensaje:', error);
        res.status(500).json({ message: 'Error al enviar el mensaje', error: error.toString() });
    }
};